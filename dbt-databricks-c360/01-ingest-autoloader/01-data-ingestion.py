...
def incrementally_ingest_folder(path, format, table):
    (spark.readStream
              ...
              .option("cloudFiles.schemaLocation", f"/dbdemos/dbt-retail-dv/_schemas/{table}")
              .load(path)
           .writeStream
              .format("delta")
              .option("checkpointLocation", f"/dbdemos/dbt-retail-dv/_checkpoints/{table}")
              .trigger(availableNow = True)
              .outputMode("append")
              .toTable(table))

spark.sql('create database if not exists field_demos.dv_dev;')
incrementally_ingest_folder('/dbdemos/dbt-retail-dv/users', 'json', 'field_demos.dv_dev.dbt_c360_bronze_users')
incrementally_ingest_folder('/dbdemos/dbt-retail-dv/orders', 'json', 'field_demos.dv_dev.dbt_c360_bronze_orders')
incrementally_ingest_folder('/dbdemos/dbt-retail-dv/events', 'csv', 'field_demos.dv_dev.dbt_c360_bronze_events')
